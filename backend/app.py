...
# ============ 数据库配置 ============
DB_PATH = Path(__file__).parent / "emotion_data.db"
...
def init_database():
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_user_date ON emotion_records(user_id, date)')
...
class EmotionSaveRequest(BaseModel):
    record: EmotionRecord

class EmotionBatchSaveRequest(BaseModel):
...
async def get_greeting():
    ...
    greetings = [
        ...
        f"嗨嗨～ {time_str}！🌟 我是小彩虹，你的情绪小伙伴～ 今天过得怎么样呀？ 💖",
    ]
...
async def get_emotions(year: int, month: int):
    """获取指定月份的情绪记录"""
    try:
        ...
        cursor.execute('''
            SELECT id, date, emotion, intensity, note
        ...
        conn.close()
        print(f"📅 获取 {year}年{month}月 情绪记录：{len(records)} 条")
        return {"code": 0, "message": "success", "data": {"records": records}}
    except Exception as e:
        print(f"❌ 获取情绪记录失败：{e}")
        return {"code": 500, "message": str(e), "data": {"records": []}}

@app.post("/api/emotions")
async def save_emotion(request: EmotionSaveRequest):
    """保存单条情绪记录"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if existing:
            ...
            cursor.execute('''
                ...
                SET emotion = ?, intensity = ?, note = ?
                WHERE date = ?
            ''', (request.record.emotion, request.record.intensity, request.record.note, request.record.date))
            print(f"📝 更新情绪记录：{request.record.date}")
        else:
            # 插入新记录
            cursor.execute('''
                INSERT OR REPLACE INTO emotion_records (id, date, emotion, intensity, note)
                VALUES (?, ?, ?, ?, ?)
            ''', (request.record.id, request.record.date, request.record.emotion, request.record.intensity, request.record.note))
            print(f"📝 保存情绪记录：{request.record.date}")

        conn.commit()
        conn.close()
        return {"code": 0, "message": "success", "data": {"saved": True}}
    except Exception as e:
        print(f"❌ 保存情绪记录失败：{e}")
        return {"code": 500, "message": str(e), "data": {"saved": False}}

@app.post("/api/emotions/batch")
async def save_emotions_batch(request: EmotionBatchSaveRequest):
    """批量保存情绪记录"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        for record in request.records:
            cursor.execute('''
                INSERT OR REPLACE INTO emotion_records (id, date, emotion, intensity, note)
                VALUES (?, ?, ?, ?, ?)
            ''', (record.id, record.date, record.emotion, record.intensity, record.note))
            saved_count += 1

        conn.commit()
        conn.close()
        print(f"📝 批量保存情绪记录：{saved_count} 条")
        return {"code": 0, "message": "success", "data": {"saved_count": saved_count}}
    except Exception as e:
        print(f"❌ 批量保存情绪记录失败：{e}")
        return {"code": 500, "message": str(e), "data": {"saved_count": 0}}
...
async def delete_emotions():
    ...
    try:
        ...
        cursor.execute('DELETE FROM emotion_records')
        deleted_count = cursor.rowcount

        conn.commit()
        conn.close()
        print(f"🗑️ 删除情绪记录：{deleted_count} 条")
        return {"code": 0, "message": "success", "data": {"deleted_count": deleted_count}}
    except Exception as e:
        print(f"❌ 删除情绪记录失败：{e}")
        return {"code": 500, "message": str(e), "data": {"deleted_count": 0}}

@app.get("/api/emotions/all")
async def get_all_emotions():
    ...
    try:
        ...
        conn.close()
        return {"code": 0, "message": "success", "data": {"records": records, "total": len(records)}}
    except Exception as e:
        print(f"❌ 获取所有情绪记录失败：{e}")
        return {"code": 500, "message": str(e), "data": {"records": [], "total": 0}}
